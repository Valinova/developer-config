...
2. Planning stage, by mode. Full: if no still-valid reviewed
   plan exists, run the Claude `$agentplan` workflow and commit the mutually
...
3. Run the Claude `$execute-plan` workflow through every phase and commit
   (Iterative: implement with the user in bounded passes instead).
4. Run the Claude `$rev` workflow and commit its reviewed fixes.
...
7. Run the Claude `$babysit` workflow on the opened pull request: wait for CI
