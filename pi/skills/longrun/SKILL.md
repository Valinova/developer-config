...
in `model-selection.md`). Codex `Agent` implements by default; review follows
"External calls". Anthropic is used only when the user names it.
...
2. Run the planning stage per mode: Full — the Pi `agentplan`
...
3. Run the Pi `execute-plan` workflow through every phase (Iterative:
   implement with the user in bounded passes) and commit.
4. Run the Pi `rev` workflow and commit its reviewed fixes.
...
7. Run the Pi `babysit` workflow on the opened pull request: wait for CI
