...
- **Explicit direction IS the approval — don't re-ask for what I already named.** A directed sequence ("stage, commit, push, then switch to X") authorizes the whole sequence: execute it end to end and report. The ask-first rules below govern operations I did **not** name. If a directed sequence needs an unnamed extra step (a stash, a rebase, a branch I didn't mention), do the named parts and ask about that one step only.
