...
2. Run the planning stage per mode: Full — the Grok `$agentplan`
...
3. Run the Grok `$execute-plan` workflow through every phase (Iterative:
   implement with the user in bounded passes) and commit.
4. Run the Grok `$rev` workflow and commit its reviewed fixes.
...
7. Run the Grok `$babysit` workflow on the opened pull request: wait for CI
