...
5. **Brand assets that already exist** - logo, color, type, photography. For redesigns, these are starting material, not optional input (see Section 10).
...
| Level | DESIGN_VARIANCE | MOTION_INTENSITY | VISUAL_DENSITY |
|---|---|---|---|
| 1-3 | Predictable: symmetrical 12-col grid (equal fr-units), equal paddings, centered alignment | Static: no automatic animation, CSS `:hover` / `:active` only | Art Gallery: huge section gaps (`py-32` to `py-48`), expensive and clean |
| 4-7 | Offset: `margin-top: -2rem` overlaps, varied aspect ratios (4:3 next to 16:9), left-aligned headers over center-aligned data | Fluid CSS: `transition: all 0.3s cubic-bezier(0.16, 1, 0.3, 1)`, `animation-delay` cascades for load-ins, `transform` / `opacity` only | Daily App: standard spacing (`py-16` to `py-24`) |
| 8-10 | Asymmetric: masonry, fractional grids (`grid-template-columns: 2fr 1fr 1fr`), massive empty zones (`padding-left: 20vw`) | Advanced Choreography: scroll-triggered reveals, parallax, scroll-driven animation (CSS `animation-timeline` or GSAP ScrollTrigger), Motion hooks | Cockpit: tight paddings, no card boxes, 1px lines separate data, `font-mono` for all numbers |

**Mobile override:** at `DESIGN_VARIANCE` 4-10, asymmetric layouts above `md:` MUST collapse to strict single-column (`w-full`, `px-4`, `py-8`) below `768px`.
...
**Honesty rule:** if the brief reads as one of the systems above, use the **official** package (ask the user before adding a dependency). Do not recreate its CSS by hand. Do not import a system's tokens but then override 90% of them.
...
* **NEVER hand-roll SVG icons.** If a glyph is missing, compose from primitives or ask the user before adding a second icon library - do not draw icon paths from scratch.
...
Before importing ANY 3rd-party library, check `package.json`. If the package is missing, ask the user before adding the dependency and show the install command. **Never** assume a library exists.
...
* **Optional depth treatment - "double bezel":** when a card genuinely earns elevation and the brief is premium, nest it like machined hardware: an outer shell (`bg-black/5` or `bg-white/5`, hairline `ring-1 ring-black/5` / `border-white/10`, `p-1.5`-`p-2`, large radius) holding an inner core with its own background, an inset top highlight (`shadow-[inset_0_1px_1px_rgba(255,255,255,0.15)]`), and a concentric smaller radius (`rounded-[calc(2rem-0.375rem)]`). Counts toward the shape lock below.
* **Optional CTA detail - nested trailing icon:** a trailing arrow can sit in its own small circle (`w-8 h-8 rounded-full bg-black/5`) flush with the button's inner padding, nudging diagonally on `group-hover`.
...
* **GSAP Sticky-Stack Pattern (when scroll-stack is used).** A "card stack on scroll" must be a REAL sticky-stack, not a sequential reveal list. Common failure: trigger fires halfway through scroll instead of pinning at viewport top. Fix: `start: "top top"` not `start: "top center"` or `"top 80%"`; pin every card except the last, and drive each card's scale/opacity from the NEXT card's scrubbed trigger so the previous card shrinks as the next arrives.
* **GSAP Horizontal-Pan Pattern (when horizontal scroll-hijack is used).** Common failure: animation starts before the section is pinned, so the user sees half a slide. Same fix: `start: "top top"`, pin the wrapper, scrub the inner track, and set `end` to the horizontal travel (`+=${track.scrollWidth - innerWidth}`).
* **Simple scroll reveals** (no pinning): prefer Motion's `whileInView` with `viewport={{ once: true }}`, gated by `useReducedMotion()`, over GSAP. Save GSAP for real pin/scrub work.
...
### 7.B Do Not Prescribe Specific Colors Here
...
### 7.D Check Both Modes Before Finishing
Do not ship a page verified in only one mode. Check the tokens and contrast for both in code first; ask the user before starting a browser session to view them.
...
### 8.D Content & Data ("Jane Doe" Effect)
...
### 8.E External Resources & Components
...
### 8.F Production-Test Tells (banned outright)
...
### 8.G EM-DASH BAN (the single most-violated Tell)
...
## 9. REFERENCE VOCABULARY (Pattern Names the Agent Should Know)
...
### 10.D Modernisation Levers (priority order)
...
5. **Hero & key-section recomposition** - restructure top-of-funnel using Section 9 vocabulary.
6. **Full block replacement** - only when the existing block is unsalvageable.

### 10.E Decision Tree: Targeted Evolution vs Full Redesign
* IA, content, and SEO sound → **targeted evolution** (Levers 1-4). ~70% of value at ~40% of risk.
* Visual debt is structural (broken IA, no design system, broken mobile) → **full redesign** with strict content preservation.
* Brand itself is changing → **greenfield**.

### 10.F What Never Changes Silently
...
- [ ] **Every mandatory rule in Section 4 and every Tell in Section 8** checked against the output (those sections are the checklist; their items are not repeated here).
- [ ] **Brief inference** declared (Section 0.B one-liner)?
- [ ] **Dial values** explicit and reasoned from the brief, not silently using baseline?
- [ ] **Design system** chosen from Section 2 if applicable, or aesthetic labeled honestly? One system per project (no Material + shadcn mixed)?
- [ ] **Redesign mode** detected and audit performed (if applicable, Section 10)?
...
- [ ] **GSAP sticky-stack / horizontal-pan** pins at `start: "top top"` with the correct scrub (Section 5)?
...
- [ ] **GSAP / Three.js instances** torn down on unmount?
- [ ] **Motion** isolated in client-leaf components with `'use client'` at the top, memoized?
- [ ] **Dark mode** tokens defined for both modes?
...
## Appendix - Apple Liquid Glass: Honest Web Approximation

Apple documents Liquid Glass for **Apple platforms only** (HIG → Materials, Developer Docs → Liquid Glass). There is no `liquid-glass.css` for the web. A web version is frosted-glass approximation - label it as such in comments. Core shape:

```css
/* Web approximation of Apple Liquid Glass - not an Apple-issued material */
...
Add a dark-mode variant (lower white alpha over a tinted base) and keep enough contrast without blur: `prefers-reduced-transparency` support is uneven.
